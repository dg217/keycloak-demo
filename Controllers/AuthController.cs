...
namespace keycloakIdentityProvider.Controllers
{
    [ApiController]
    public class AuthController : Controller
    {
        ...
        public AuthController(HttpClient httpClient, KeyclockApiClient keyCloakApiClient)
        {
        ...
        [HttpPost]
        [Route("clientLogin")]
        public async Task<IActionResult> keyclockLogin(ClientLogin users)
        {
            try
            {
                ...
                return Ok(new { status = 200, isSuccess = true, token = myDeserializedClass?.access_token, userole = claimValue, username = userName, });
            }
            catch
            {
        ...
        public async Task<IActionResult> keyclockRegister(Register register)
        {
            ...
            HttpResponseMessage registerRequet = _httpClient.PostAsJsonAsync("http://localhost:8080/admin/realms/test/users", register).Result;
